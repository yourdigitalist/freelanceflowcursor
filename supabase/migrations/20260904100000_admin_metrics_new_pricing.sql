-- Recalculate admin MRR/ARR at the new Lance prices: $15/mo and $150/yr.
...
DECLARE
  ...
  unconfirmed_signups integer;
  unconfirmed_signups_week integer;
  unconfirmed_signups_month integer;
  trial_users integer;
...
BEGIN
  ...
  SELECT count(*)::integer INTO unconfirmed_signups
  FROM auth.users u
  WHERE u.email_confirmed_at IS NULL
    AND coalesce(u.is_anonymous, false) = false;

  SELECT count(*)::integer INTO unconfirmed_signups_week
  FROM auth.users u
  WHERE u.email_confirmed_at IS NULL
    AND coalesce(u.is_anonymous, false) = false
    AND u.created_at >= week_start;

  SELECT count(*)::integer INTO unconfirmed_signups_month
  FROM auth.users u
  WHERE u.email_confirmed_at IS NULL
    AND coalesce(u.is_anonymous, false) = false
  ...
  mrr := (monthly_active * 15) + (annual_active * (150.0 / 12.0));
  arr := mrr * 12;
  ...
  new_mrr_month := (
    SELECT coalesce(sum(
      CASE
        WHEN lower(coalesce(plan_type, '')) = 'pro_annual' THEN 150.0 / 12.0
        ELSE 15
      END
    ), 0)
    FROM public.profiles
  ...
  RETURN jsonb_build_object(
    ...
    'unconfirmed_signups', unconfirmed_signups,
    'unconfirmed_signups_this_week', unconfirmed_signups_week,
    'unconfirmed_signups_this_month', unconfirmed_signups_month,
...
COMMENT ON FUNCTION public.get_admin_metrics() IS
  'Admin dashboard summary: users, revenue, trials, unconfirmed email signups. Admin only. MRR uses $15/mo and $150/yr.';
