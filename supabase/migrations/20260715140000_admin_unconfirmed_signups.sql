-- Admin metrics: track signups who have not confirmed their email yet.
...
COMMENT ON FUNCTION public.get_admin_metrics() IS
  'Admin dashboard summary: users, revenue, trials, unconfirmed email signups. Admin only.';

COMMENT ON FUNCTION public.get_admin_users_list() IS
  'Admin dashboard user rows with auth email confirmation status. Admin only.';
